...
public class Application1 {
	...
	public static void main(String[] args) {
		...
		/*Rabbit과 Tiger는 Animal 클래스를 상속 받아서
		 * Rabbit은 Rabbit 타입 이기도 하면서 Animal 타입이기도 하고
		 * Tiger는 Tiger 타입 이기도 하면서 Animal 타입이기도 하다
		 * ---> 다형성
		 * */
		
		Animal a1 = new Rabbit();
		Animal a2 = new Tiger(); // 묵시적 형변환 (up-casting)
		
		//Rabbit r = new Animal();
		//Tiger t = new Animal(); 불가능(에러)
		...
		// 현재 레퍼런스 변수 타입은 Animal 이기 때문에 Rabbit과 Tiger의 고유 기능을 동작 시키지 못한다.
		//a1.jump(); 
		//a2.bite();
		...
		((Rabbit) a1).jump(); // 다운캐스팅 : 하위(자식) 타입으로 변환
		((Tiger) a2).bite();
		
		//((Tiger)a1).bite(); // 컴파일 시에는 문제가 안됨, 런타임 시 ClassCastExeption이 발생
