...
public class Application1 {
	...
	public static void main(String[] args) {
		...
		//다형성이 적용되어 상위 레퍼런스로 ArrayList 객체를 만들 수 있다.
		List list = new ArrayList();
		
		Collection clist = new ArrayList();
		
		// arraylist는 저장순서가 유지되고 index 적용됨
		alist.add("apple");
		...
		// arraylist의 크기를 확인, 요소의 갯수 반환
		System.out.println(alist.size());
		
		
		// index가 지정되어있어 for문으로 접근 가능
		for (int i = 0 ; i < alist.size(); i++) {
			System.out.println(i + " : " + alist.get(i)); // 값을 가져올때 get()
		}
		...
		// 원하는 인덱스 위치에 값 추가 가능, 값을 추가하는 경우 덮어 쓰는 것이 아니고
		// 새로운 값이 들어가는 인덱스 위치에 값을 넣고 인덱스가 하나씩 뒤로 밀림.
		alist.add(1, "banana");
		System.out.println(alist);
		...
		alist.set(1, new Boolean(true)); // 지정한 인덱스 위치의 값 수정
		System.out.println(alist);
