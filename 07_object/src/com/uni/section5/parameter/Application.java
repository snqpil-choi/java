...
public class Application {
	...
	public static void main(String[] args) {
		...
		System.out.println(); // 출력 같다(얕은복사했으므로)
		...
		r1.calcArea();
		r1.calcRound(); // 얕은복사가 일어나서 같은 값이 출력된다
		
		
//		pt.testVariableLengthArrayParameter(); // 에러
		pt.testVariableLengthArrayParameter("로운"); // 가변인자는 전달하지 않아도 됨
		pt.testVariableLengthArrayParameter("로운","볼링");
		pt.testVariableLengthArrayParameter("로운","볼링","골프","서핑");
		pt.testVariableLengthArrayParameter("로운",new String [] {"볼링","골프","서핑"}); // 아예 배열입력도 가능
