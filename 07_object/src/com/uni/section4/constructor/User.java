...
//		오버로딩 : 매개변수의 타입, 개수, 순서에 따라서 생성자 혹은 메소드를 한 클래스 내에서 여러개 작성가능 
...
//		this.id = id;
//		this.pwd = pwd;
//		this.name = name; // 또 쳐야하나? 방법이 있다! 생성자를 호출한다
	...
	/* 복사 생성자 */
	/* 이미 만들어진 동일한 타입의 인스턴스가 가지는 필드 값을 이용해서 새로운 인스턴스 생성 시 초기화값으로 이용할 수 있다. 동일한 값을
	 * 가지지만 새롭게 할당되는 인스턴스이기 때문에 서로 다른 주소값을 가지게 된다. (깊은복사)
	 */
